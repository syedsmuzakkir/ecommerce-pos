...
const Vegetables = (props) => {
  ...
  const [currentView, setCurrentView] = useState('Vegetables');

  const filteredProducts = props.vegetables.filter((veg) =>
...
// console.log(leafyVegetables, 'this is leafyvegetables')
  ...
  useEffect(() => {
    if (props.vegetables.length > 0) {
      setLoading(false);
    }
  }, [props.vegetables]);

  const handleGoBack = () => {
    setCurrentView('Vegetables');
  };
