...
const LeafyVegetables = (props) => {
  console.log(props, 'this is from fruits');
  const dispatch = useDispatch();
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const reduxData = useSelector(state => state.CustomerSlice);
  const [currentView, setCurrentView] = useState('LeafyVegetables');

  const filteredProducts = props.leafyVegetables.filter((veg) =>
...
const { width, height } = Dimensions.get('window'); // Get screen dimensions
  ...
  useEffect(() => {
    if (props.leafyVegetables.length > 0) {
      setLoading(false);
    }
  }, [props.leafyVegetables]);

  const handleGoBack = () => {
    setCurrentView('LeafyVegetables');
  };
  ...
  return (
    // <View>
    //   {currentView === 'LeafyVegetables' ? (
    //     <ScrollView style={{ flex: 1, width: 760 , height:560, overflow:'scroll'  }} Vertical={true} showsVerticalScrollIndicator={true}>
